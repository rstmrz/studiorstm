...
function arlo_tm_animate_text(){
	...
	var animateSpan			= jQuery('.arlo_tm_animation_text_word');
		...
		animateSpan.typed({
			strings: ["freelance intermediaire", "passionné par le e-commerce", "créateur de valeur pour les marques"],
			loop: true,
			startDelay: 1e3,
			backDelay: 2e3
		});
...
function arlo_tm_switcher(){
	...
	switcherOpener.on('click',function(e){
		e.preventDefault();
		var scrollY = window.pageYOffset || document.documentElement.scrollTop || document.body.scrollTop || 0;

		/* Gèle la position visuelle pendant le reflow (padding + transitions) */
		...
		var $rip = jQuery('#ripple');
		if ($rip.length) {
			setTimeout(function(){ $rip.ripples('updateSize'); },101);
			setTimeout(function(){ $rip.ripples('updateSize'); },201);
			setTimeout(function(){ $rip.ripples('updateSize'); },301);
		}

		/* Évite destroy/reinit jarallax (souvent cause de saut en haut) : recalcul au resize */
		setTimeout(function(){ jQuery(window).trigger('resize').trigger('scroll'); }, 40);
		setTimeout(function(){ jQuery(window).trigger('resize').trigger('scroll'); }, 340);
